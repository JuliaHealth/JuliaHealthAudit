...
for (idx, repo_name) in enumerate(repos)
    ...
    local contrib_page = 1
    while true
        data = github_request("/repos/$ORG/$repo_name/contributors?per_page=100&page=$contrib_page")
        isnothing(data) && break
        isempty(data) && break
...
for (login, info) in contributors
    ...
    display_name = if !isnothing(profile)
        ...
        if !isnothing(profile_name) && !isempty(String(profile_name))
            String(profile_name)
        else
            login
        end
