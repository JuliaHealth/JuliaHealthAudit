...
comparison_df = DataFrame(
    category=["Packages", "Non-Packages"],
    avg_stars=[mean(packages_df.stars), mean(nonpackages_df.stars)],
    max_stars=[maximum(packages_df.stars), maximum(nonpackages_df.stars)],
)

# Just show averages
comparison_avg = comparison_df[:, [:category, :avg_stars]]
rename!(comparison_avg, :avg_stars => :avg_stars)
comparison_avg |>
@vlplot(
    :bar,
    x=:avg_stars,
...
comparison_contrib = DataFrame(
    category=["Packages", "Non-Packages"],
    avg_contributors=[mean(packages_df.contributors_count), mean(nonpackages_df.contributors_count)],
)
comparison_contrib |>
@vlplot(
    :bar,
...
comparison_resolution = DataFrame(
    category=["Package Issues", "Package PRs", "Non-Pkg Issues", "Non-Pkg PRs"],
    resolution_rate=[
        mean(packages_df.issue_resolution_rate),
        mean(packages_df.pr_resolution_rate),
        mean(nonpackages_df.issue_resolution_rate),
        mean(nonpackages_df.pr_resolution_rate),
    ],
)
